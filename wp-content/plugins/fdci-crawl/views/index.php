...
foreach($properties as $container) {
  ...
  foreach($arr as $index=>$item) {
    ...
    #get ID
   	$propId  = trim($explodedHref[count($explodedHref)-1]);
...
/*    echo "<pre>";
    ...
    $checkDataExist  = $wpdb->get_results("SELECT * FROM $tableName WHERE reference_no = '$propId'");
    $check = 0;
    foreach($checkDataExist as $rows){
      $check = 1;
    }
    if($check==0){
